...
module s9_slot1_result_packet_tb;
    logic clk;
    logic rst_n;
    integer checks;
    integer failures;
    ...
    task automatic drive_payload(input logic valid, input logic issue_valid);
        begin
            ...
            slot1_mem_lsu = valid && issue_valid;
    ...
    initial begin
        ...
        #1;
        `check32("PKT invalid still passes pc for debug", slot1_pc_o, 32'h1c00_9004);
        ...
        `check("PKT invalid clears LSU ownership", !slot1_mem_lsu_o);
        ...
        `check("PKT valid without issue clears LSU ownership", !slot1_mem_lsu_o);
        ...
        tlb_badv = 32'h4000_1234;
        #1;
        `check("PKT valid TLB exception pass",
               slot1_exception_o && slot1_excp_ecode_o == 6'h3f &&
               slot1_excp_esubcode_o == 9'd0 && slot1_excp_badv_valid_o);
        `check32("PKT valid TLB badv pass", slot1_excp_badv_o, 32'h4000_1234);
        ...
        slot1_excp_badv = 32'h1000_0062;
        #1;
        `check("PKT existing exception wins over TLB ecode",
               slot1_exception_o && slot1_excp_ecode_o == 6'h09 &&
               slot1_excp_esubcode_o == 9'h012);
        `check32("PKT existing badv wins over TLB badv", slot1_excp_badv_o, 32'h1000_0062);
        ...
        `check("PKT no exception can preserve explicit badv valid",
        ...
        `check("PKT valid issued memory carries LSU ownership", slot1_mem_lsu_o);
        ...
        #1;
        `check("PKT non-memory cannot carry LSU ownership", !slot1_mem_lsu_o);

        slot1_is_load = 1'b1;
        slot1_issue_class = 4'b1000;
        #1;
        `check("PKT ignores stale memory payload without memory class",
               !slot1_mem_lsu_o);
