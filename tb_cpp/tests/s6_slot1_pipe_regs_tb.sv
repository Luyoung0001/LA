...
module s6_slot1_pipe_regs_tb;
    ...
    initial begin
        ...
        rst_n = 1'b1;
        capture = 1'b0;
        drive_payload(32'h1c00_6004, 1'b1, 1'b1);

        capture = 1'b1;
        tick();
        #1;
        `check32("S6 slot1 regs capture pc", slot1_pc_o, 32'h1c00_6004);
        `check32("S6 slot1 regs capture instr", instr1_o, 32'h1c00_6004 ^ 32'h2288_0018);
        `check("S6 slot1 regs capture valid bits", slot1_valid_o && slot1_issue_valid_o);
        `check32("S6 slot1 regs capture register fields",
                 {17'd0, slot1_rs1_o, slot1_rs2_o, slot1_rd_o},
                 {17'd0, 5'd4, 5'd0, 5'd24});
        `check32("S6 slot1 regs capture imm", slot1_imm_o, 32'h1c00_6104);
        `check32("S6 slot1 regs capture rs1 value", slot1_rs1_val_o, 32'h2c00_6005);
        `check32("S6 slot1 regs capture issue class",
        ...
        tick();
        #1;
        `check32("S6 slot1 regs hold pc when capture low", slot1_pc_o, 32'h1c00_6004);
        `check("S6 slot1 regs hold valid bits when capture low", slot1_valid_o && slot1_issue_valid_o);
        `check32("S6 slot1 regs hold rs2 value when capture low", slot1_rs2_val_o, 32'h3c00_6006);
        `check32("S6 slot1 regs hold issue class when capture low",
                 {28'd0, slot1_issue_class_o}, {28'd0, 4'b1010});

        capture = 1'b1;
        tick();
        #1;
        `check32("S6 slot1 regs recapture pc", slot1_pc_o, 32'h1c00_9008);
        `check("S6 slot1 regs recapture invalid bits", !slot1_valid_o && !slot1_issue_valid_o);
        `check32("S6 slot1 regs recapture invalid clears issue class", {28'd0, slot1_issue_class_o}, 32'd0);
        `check32("S6 slot1 regs recapture rs1 value", slot1_rs1_val_o, 32'h2c00_9009);
        `check32("S6 slot1 regs recapture decode sideband",
