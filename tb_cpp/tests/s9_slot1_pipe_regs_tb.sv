...
module s9_slot1_pipe_regs_tb;
    ...
    initial begin
        ...
        tick();
        #1;
        `check32("S9 slot1 regs capture pc", slot1_pc_o, 32'h1c00_a018);
        `check32("S9 slot1 regs capture instr", instr1_o, 32'h1c00_a018 ^ 32'h5909_0019);
        `check("S9 slot1 regs capture valid bits", slot1_valid_o && slot1_issue_valid_o);
        `check32("S9 slot1 regs capture issue class", {28'd0, slot1_issue_class_o}, {28'd0, 4'b0101});
        `check("S9 slot1 regs capture LSU ownership", slot1_mem_lsu_o);
        `check32("S9 slot1 regs capture alu result", slot1_alu_result_o, 32'h4c00_a01b);
        `check32("S9 slot1 regs capture store data", slot1_store_data_o, 32'h5c00_a01c);
        `check32("S9 slot1 regs capture exception badv", slot1_excp_badv_o, 32'h6c00_a01d);

        capture = 1'b0;
        drive_payload(32'h1c00_b004, 1'b0, 1'b0);
        tick();
        #1;
        `check32("S9 slot1 regs hold pc when capture low", slot1_pc_o, 32'h1c00_a018);
        `check("S9 slot1 regs hold valid bits when capture low", slot1_valid_o && slot1_issue_valid_o);
        `check32("S9 slot1 regs hold issue class when capture low", {28'd0, slot1_issue_class_o}, {28'd0, 4'b0101});
        `check("S9 slot1 regs hold LSU ownership when capture low", slot1_mem_lsu_o);
        `check32("S9 slot1 regs hold mem size when capture low", {30'd0, slot1_mem_size_o}, 32'd0);

        capture = 1'b1;
        tick();
        #1;
        `check32("S9 slot1 regs recapture pc", slot1_pc_o, 32'h1c00_b004);
        `check("S9 slot1 regs recapture invalid bits", !slot1_valid_o && !slot1_issue_valid_o);
        `check32("S9 slot1 regs recapture invalid clears issue class", {28'd0, slot1_issue_class_o}, 32'd0);
        `check("S9 slot1 regs recapture invalid clears LSU ownership", !slot1_mem_lsu_o);
        `check32("S9 slot1 regs recapture imm", slot1_imm_o, 32'h1c00_b244);
        `check32("S9 slot1 regs recapture opcode", {25'd0, slot1_opcode_o}, {25'd0, 7'h04});
