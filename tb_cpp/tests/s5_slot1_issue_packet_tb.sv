...
module s5_slot1_issue_packet_tb;
    logic clk;
    logic rst_n;
    integer checks;
    integer failures;
    ...
    task automatic drive_payload(input logic valid, input logic issue_valid);
        begin
            slot1_pc = 32'h1c00_1004;
            instr1 = 32'h0280_0405;
            slot1_valid = valid;
            slot1_opcode = 7'h2a;
            slot1_funct3 = 3'h5;
            slot1_funct7 = 7'h12;
            slot1_op_19_15 = 5'h0a;
            slot1_rs1 = 5'd3;
            slot1_rs2 = 5'd4;
            slot1_rd = 5'd5;
            slot1_imm = 32'h0000_0123;
            ...
            slot1_uses_rs1 = 1'b1;
            slot1_uses_rs2 = 1'b1;
            slot1_src_a_sel = 2'd1;
            slot1_src_b_sel = 2'd2;
            slot1_alu_op = 4'h7;
            slot1_rs1_val = 32'h1111_2222;
            slot1_rs2_val = 32'h3333_4444;
    ...
    initial begin
        ...
        #1;
        `check32("PKT invalid still passes pc for debug", slot1_pc_o, 32'h1c00_1004);
        `check32("PKT invalid still passes instr for debug", instr1_o, 32'h0280_0405);
        `check("PKT invalid clears valid and issue", !slot1_valid_o && !slot1_issue_valid_o);
        `check("PKT invalid clears control bits",
               !slot1_is_branch_o && !slot1_wb_en_o && !slot1_uses_rs1_o && !slot1_uses_rs2_o);
        ...
        `check32("PKT invalid clears payload selectors", {26'd0, slot1_src_a_sel_o, slot1_src_b_sel_o, slot1_alu_op_o}, 32'd0);
        ...
        `check("PKT valid keeps branch and source usage", slot1_is_branch_o && slot1_wb_en_o && slot1_uses_rs1_o && slot1_uses_rs2_o);
        `check32("PKT valid opcode", {25'd0, slot1_opcode_o}, {25'd0, 7'h2a});
        `check32("PKT valid funct bundle", {17'd0, slot1_funct3_o, slot1_funct7_o, slot1_op_19_15_o}, {17'd0, 3'h5, 7'h12, 5'h0a});
        `check32("PKT valid registers", {17'd0, slot1_rs1_o, slot1_rs2_o, slot1_rd_o}, {17'd0, 5'd3, 5'd4, 5'd5});
        `check32("PKT valid imm", slot1_imm_o, 32'h0000_0123);
        `check32("PKT valid selectors", {26'd0, slot1_src_a_sel_o, slot1_src_b_sel_o, slot1_alu_op_o}, {26'd0, 2'd1, 2'd2, 4'h7});
        `check32("PKT valid rs1 value", slot1_rs1_val_o, 32'h1111_2222);
        `check32("PKT valid rs2 value", slot1_rs2_val_o, 32'h3333_4444);

        drive_payload(1'b1, 1'b1);
        #1;
        `check("PKT valid and issue emits issue", slot1_valid_o && slot1_issue_valid_o);
        `check32("PKT valid and issue preserves issue class", {28'd0, slot1_issue_class_o}, {28'd0, 4'b1000});
