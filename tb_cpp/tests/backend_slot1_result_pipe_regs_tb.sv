...
module backend_slot1_result_pipe_regs_tb;
    logic clk;
    logic rst_n;
    integer checks;
    integer failures;
    ...
    backend_slot1_result_pipe_regs u_dut (
        .clk(clk),
        .capture_i(capture),
        .slot1_pc_i(slot1_pc),
        .instr1_i(instr1),
    ...
    initial begin
        ...
        tick();
        #1;
        `check32("REGS capture pc", slot1_pc_o, 32'h1c00_a018);
        `check("REGS capture valid bits", slot1_valid_o && slot1_issue_valid_o);
        `check32("REGS capture issue class", {28'd0, slot1_issue_class_o}, {28'd0, 4'b1010});
        `check("REGS capture memory kind", slot1_is_load_o && slot1_is_store_o);
        `check("REGS capture explicit LSU ownership", slot1_mem_lsu_o);
        `check32("REGS capture alu result", slot1_alu_result_o, 32'h4c00_a01b);
        `check32("REGS capture store data", slot1_store_data_o, 32'h5c00_a01c);
        `check32("REGS capture exception badv", slot1_excp_badv_o, 32'h6c00_a01d);

        capture = 1'b0;
        drive_payload(32'h1c00_b004, 1'b0, 1'b0);
        tick();
        #1;
        `check32("REGS hold pc when capture low", slot1_pc_o, 32'h1c00_a018);
        `check("REGS hold valid bits when capture low", slot1_valid_o && slot1_issue_valid_o);
        `check32("REGS hold issue class when capture low", {28'd0, slot1_issue_class_o}, {28'd0, 4'b1010});
        `check("REGS hold LSU ownership when capture low", slot1_mem_lsu_o);
        `check32("REGS hold mem size when capture low", {30'd0, slot1_mem_size_o}, 32'd0);

        capture = 1'b1;
        tick();
        #1;
        `check32("REGS recapture pc", slot1_pc_o, 32'h1c00_b004);
        ...
        `check("REGS recapture invalid clears LSU ownership", !slot1_mem_lsu_o);
        `check32("REGS recapture imm", slot1_imm_o, 32'h1c00_b204);
        `check32("REGS recapture opcode", {25'd0, slot1_opcode_o}, {25'd0, 7'h04});
