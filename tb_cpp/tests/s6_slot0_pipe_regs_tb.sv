...
module s6_slot0_pipe_regs_tb;
    ...
    task automatic check_payload;
        ...
        begin
            ...
            `check32({prefix, " instr"}, instr_o, base ^ 32'h0f0f_3300);
            `check32({prefix, " epoch/pred/fault"},
                     {28'd0, epoch_o, pred_taken_o, fault_o},
                     {28'd0, base[1:0], base[2], base[3]});
            `check32({prefix, " pred target"}, pred_target_o, base + 32'h40);
            ...
            `check32({prefix, " rs1 value"}, rs1_val_o, base + 32'h0100_0001);
            `check32({prefix, " rs2 value"}, rs2_val_o, base + 32'h0200_0002);
    ...
    initial begin
        ...
        tick();
        #1;
        check_payload("S6S0REG capture", 32'h1c00_4104);

        capture = 1'b0;
        drive_payload(32'h1c00_5228);
        tick();
        #1;
        check_payload("S6S0REG hold", 32'h1c00_4104);

        capture = 1'b1;
        tick();
        #1;
        check_payload("S6S0REG recapture", 32'h1c00_5228);
