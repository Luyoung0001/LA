...
module s6_slot0_issue_packet_tb;
    logic clk;
    logic rst_n;
    integer checks;
    integer failures;
    ...
    task automatic drive_payload(input logic valid);
        begin
            ...
            epoch = 2'd1;
            pred_taken = 1'b0;
            pred_target = 32'h1c00_3040;
            fault = 1'b0;
            fault_ecode = 6'h05;
            fault_badv = 32'h4000_3000;
            rs1 = 5'd8;
            rs2 = 5'd9;
            rd = 5'd10;
            imm = 32'h0000_0100;
            ...
            is_load = 1'b1;
            is_store = 1'b0;
            rs1_val = 32'h0101_0202;
            rs2_val = 32'h0303_0404;
            mem_size = 2'b01;
            mem_sign_ext = 1'b1;
            src_a_sel = 2'd0;
            src_b_sel = 2'd1;
    ...
    task automatic check_payload(input string prefix, input logic valid);
        begin
            ...
            `check32({prefix, " pc"}, pc_o, 32'h1c00_3000);
            `check32({prefix, " instr"}, instr_o, 32'h0280_0821);
            `check32({prefix, " epoch/pred/fault"}, {28'd0, epoch_o, pred_taken_o, fault_o},
                     {28'd0, 2'd1, 1'b0, 1'b0});
            `check32({prefix, " pred target"}, pred_target_o, 32'h1c00_3040);
            `check32({prefix, " fault bundle"}, {18'd0, fault_ecode_o, fault_badv_o[7:0]},
                     {18'd0, 6'h05, 8'h00});
            `check32({prefix, " registers"}, {17'd0, rs1_o, rs2_o, rd_o},
                     {17'd0, 5'd8, 5'd9, 5'd10});
            `check32({prefix, " imm"}, imm_o, 32'h0000_0100);
            `check({prefix, " controls"},
                   wb_en_o && uses_rs1_o && uses_rs2_o && !is_branch_o &&
                   !is_jal_o && !is_jalr_o && is_load_o && !is_store_o);
            `check32({prefix, " rs1 value"}, rs1_val_o, 32'h0101_0202);
            `check32({prefix, " rs2 value"}, rs2_val_o, 32'h0303_0404);
            `check32({prefix, " exec selectors"},
                     {20'd0, mem_size_o, mem_sign_ext_o, src_a_sel_o, src_b_sel_o, alu_op_o, branch_op_o},
                     {20'd0, 2'b01, 1'b1, 2'd0, 2'd1, 4'h0, 3'h1});
