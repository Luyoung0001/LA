...
module backend_slot1_decode_pipe_regs_tb;
    logic clk;
    logic rst_n;
    integer checks;
    integer failures;
    ...
    backend_slot1_decode_pipe_regs u_dut (
        .clk(clk),
        .capture_i(capture),
        .slot1_pc_i(slot1_pc),
        .instr1_i(instr1),
    ...
    initial begin
        ...
        tick();
        #1;
        `check32("REGS capture pc", slot1_pc_o, 32'h1c00_6004);
        `check32("REGS capture instr", instr1_o, 32'h1c00_6004 ^ 32'h2288_0018);
        ...
        `check32("REGS capture imm", slot1_imm_o, 32'h1c00_6104);
        `check32("REGS capture rs values", slot1_rs1_val_o, 32'h2c00_6005);
        ...
        tick();
        #1;
        `check32("REGS hold pc when capture low", slot1_pc_o, 32'h1c00_6004);
        `check("REGS hold valid bits when capture low", slot1_valid_o && slot1_issue_valid_o);
        `check32("REGS hold rs2 when capture low", slot1_rs2_val_o, 32'h3c00_6006);
