...
// -----------------------------------------------------------------------------
// Multi-cycle Booth/Wallace multiplier implementation.
// These modules are private to mul.v and were grouped here to keep RTL tidy.
// -----------------------------------------------------------------------------

module TopMultiplier (
  input         clk,
  input         reset,
  input         mult,          // start pulse
  input  [31:0] x_in,
  input  [31:0] y_in,
  input         signed_op,     // 1=signed multiply, 0=unsigned multiply
  output reg [63:0] result_out,
  output reg    done           // one-cycle result valid pulse
);
  ...
  always @(posedge clk or posedge reset) begin
    if (reset) begin
      ...
      case (state)
        ...
        BOOTH: begin
          // Booth outputs are ready for Wallace compression.
          state <= WALLACE;
        end

        WALLACE: begin
          // Latch Wallace outputs.
          opa_reg <= opa;
          opb_reg <= opb;
          state <= SIGN_COMP;
        end
...
module BoothWrapper(
    ...
    // Instantiate both signed and unsigned Booth encoders.
    wire [31:0] c_pp [0:31];
    wire [31:0] u_pp [0:31];
    wire [31:0] c_sign;
...
module WallaceTree32x32 (
    // 32 by 32 partial products.
    input  [31:0] pp0,  pp1,  pp2,  pp3,  pp4,  pp5,  pp6,  pp7,
                   pp8,  pp9,  pp10, pp11, pp12, pp13, pp14, pp15,
                   pp16, pp17, pp18, pp19, pp20, pp21, pp22, pp23,
                   pp24, pp25, pp26, pp27, pp28, pp29, pp30, pp31,
    // Two 64-bit operands for the final carry-propagate adder.
    output [63:0] opa,
    output [63:0] opb/*
    output [63:0] op1,
    output [63:0] op2,
